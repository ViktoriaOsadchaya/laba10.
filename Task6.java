    ...
    public static void main(String[] args) throws IOException {
        ...
        while(fread.ready()){
            ...
            if (Double.parseDouble(mas[2])<niztemp){
                niztemp=Double.parseDouble(mas[2]);nizGor=mas[0];
            }
            if (Double.parseDouble(mas[2])>visTemp){
                visTemp=Double.parseDouble(mas[2]);visGor=mas[0];
            }
            ...
            if (cnt%11 == 0&cnt!=0){
                ...
                fWr.write(k+") "+mas[0]+" Среднегодовые осадки = "+mm.format(m)+" Среднегодовая температура = "+mm.format(c)+"\r\n\r\n");
                srTEMP=0.0; srOsadki=0.0;cnt=0;
                k++;
            }
            line=fread.readLine();
            cnt++;
